//Problem Link: https://open.kattis.com/problems/shares
#include <bits/stdc++.h>
using namespace std;
...
int main() {
    ...
    for (int i=0; i<N; ++i) cin >> share[i].first >> share[i].second;
    ...
    for (int i=0, r; i<P; ++i) {
        ...
        if (pack[i].first <= C and pack[i].second > 0) packs.push_back(pack[i]);
